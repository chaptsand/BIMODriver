...
class combine_net_gate_without_ac(torch.nn.Module):
    ...
    def forward(self, x, ppi_edge, L_emb, L_emb_edge, contrastive_mask=None,
        ...
        """Run BIMODriver on the supplied fold-specific graph.

        ``input_node_mask`` is used only by strict-inductive training.  Rows
        outside the mask are replaced by zeros in every input modality before
        any trainable layer sees them.  The original tensors are not mutated,
        so the complete pre-computed features remain available for inference.
        """
        if input_node_mask is not None:
            input_node_mask = input_node_mask.to(x.device).bool()
            if input_node_mask.ndim != 1 or input_node_mask.numel() != x.size(0):
                raise ValueError('input_node_mask must contain one entry per node')
            x = x.masked_fill(~input_node_mask.unsqueeze(1), 0)
            L_emb = {
                name: features.masked_fill(
                    ~input_node_mask.to(features.device).unsqueeze(1), 0
                )
