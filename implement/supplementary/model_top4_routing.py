...
def save_best_epoch_results(
    ...
    if all_aurocs.shape[0] < epochs or all_auprcs.shape[0] < epochs:
        raise ValueError("metric arrays must contain one row per epoch")

    selected_epochs = [i for i in range(epochs) if (i + 1) % 10 == 0]
    # Preserve the original every-10-epochs selection rule, while also making
    # short runs valid and ensuring the actual final epoch can be compared.
    ...
    final_auroc_matrix = all_aurocs[final_epoch]
    final_auprc_matrix = all_auprcs[final_epoch]
    ...
    with open(path, 'a') as f:
        ...
        f.write(
            f"# Final Epoch: {final_epoch + 1} | "
            f"Mean AUROC: {final_auroc_matrix.mean():.4f} +- "
            f"{final_auroc_matrix.std():.4f} | "
            f"Mean AUPRC: {final_auprc_matrix.mean():.4f} +- "
            f"{final_auprc_matrix.std():.4f}\n"
        )
        f.write("Final AUROC matrix:\n")
        np.savetxt(f, final_auroc_matrix, fmt='%.6f')
        f.write("Final AUPRC matrix:\n")
        np.savetxt(f, final_auprc_matrix, fmt='%.6f')
        f.write(
            f"# Best-vs-final | Same epoch: {best_epoch == final_epoch} | "
            f"AUROC delta (best-final): "
            f"{best_auroc_matrix.mean() - final_auroc_matrix.mean():+.6f} | "
            f"AUPRC delta (best-final): "
            f"{best_auprc_matrix.mean() - final_auprc_matrix.mean():+.6f}\n"
        )
class combine_net_gate_without_ac(torch.nn.Module):
    ...
    def forward(self, x, ppi_edge, L_emb, L_emb_edge, return_routing=False,
                contrastive_mask=None, input_node_mask=None):
        """Run the mixture-of-experts model.

        ``input_node_mask`` implements strict inductive training without
        mutating the pre-computed feature tensors. Rows outside the mask are
        replaced by zeros in both the omics/structural features and every
        frozen language-feature view. At inference it is omitted, restoring
        the original frozen features together with the complete graphs.
        """
        if input_node_mask is not None:
            input_node_mask = input_node_mask.to(device=x.device, dtype=torch.bool)
            if input_node_mask.ndim != 1 or input_node_mask.numel() != x.size(0):
                raise ValueError("input_node_mask must contain one value per node")
            feature_mask = input_node_mask.unsqueeze(1)
            x = x.masked_fill(~feature_mask, 0.0)
            L_emb = {
                name: features.masked_fill(~feature_mask, 0.0)
                for name, features in L_emb.items()
            }
        ...
        if self.training:
            if contrastive_mask is not None:
                input_G_loss = input_G[contrastive_mask]
                input_self_loss = input_self[contrastive_mask]
                input_together_loss = input_together[contrastive_mask]
            else:
                input_G_loss = input_G
                input_self_loss = input_self
                input_together_loss = input_together
            loss_inter = diagonal_contrastive_loss(input_G_loss, input_self_loss, tau=0.04) + diagonal_contrastive_loss(input_self_loss, input_together_loss, tau=0.04) + diagonal_contrastive_loss(input_G_loss, input_together_loss, tau=0.04)
        else:
        ...
        topk_logits, topk_indices = torch.topk(gating_score, k=self.top_k, dim=1)
        ...
        # Scatter the Top-4 softmax weights back to all six experts.  The two
        # experts outside the Top-4 are exactly zero, while every row still
        # sums to one.  This is the probability definition exported for plots.
