# -*- coding: utf-8 -*-
"""Unchanged BIMODriver network used by both Table A17 variants."""
import os
import numpy as np
import pandas as pd
import time
...
def diagonal_contrastive_loss(h1, h2, tau=0.1, node_mask=None):
    """InfoNCE loss over the selected nodes only.

    In the strict inductive experiment ``node_mask`` excludes held-out genes
    before the similarity matrix is constructed.  Consequently, test genes
    are neither positive pairs nor negatives for any training node.
    """
    if node_mask is not None:
        node_mask = node_mask.to(device=h1.device, dtype=torch.bool)
        h1 = h1[node_mask]
        h2 = h2[node_mask]
    if h1.size(0) == 0:
        return (h1.sum() + h2.sum()) * 0.0

    sim_matrix = sim(h1, h2)  # [N_selected, N_selected]
...
def save_best_epoch_results(
    ...
    epochs=150,txt='none', output_path=None
):
    res_dir = REPO_ROOT / 'result'
    res_dir.mkdir(parents=True, exist_ok=True)
    if output_path is not None:
        path = output_path
    elif cancerType == 'pan-cancer':
        if dataset == 'cpdb':
