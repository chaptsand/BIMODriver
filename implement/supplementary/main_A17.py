# -*- coding: utf-8 -*-
"""Table A17: original and strict edge-masking BIMODriver experiments."""
import numpy as np
import pandas as pd
import time
import pickle
...
from model_A17 import *


RESULT_ROOT = str(REPO_ROOT / 'result' / 'A17')
STRICT_MASKING = os.environ.get('A17_STRICT_MASKING', '0') == '1'
EXPERIMENT_VARIANT = 'masking' if STRICT_MASKING else 'original'
...
def create_run_result_dir(cancerType, dataset, lr, dropout, lambdinter, epochs):
    ...
    run_name = (
        f'{dataset}_{cancerType}_{EXPERIMENT_VARIANT}_{timestamp}'
    ...
    with open(metadata_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f'variant={EXPERIMENT_VARIANT}\n')
        f.write(f'strict_masking={STRICT_MASKING}\n')
...
def save_results_to_file(auroc, auprc, cancerType, dataset='cpdb', lr=0.001, dropout=0.2, lambdinter=0.005):
    ...
    path = os.path.join(RESULT_ROOT, f'{dataset}_{EXPERIMENT_VARIANT}_summary.txt')
...
def filter_edges_incident_to_test(edge_index, test_mask):
    """Remove exactly the edges whose source or destination is a test gene."""
    src, dst = edge_index
...
def train_test(data_model, optimizer, data, L_emb, edge_index, L_emb_edge,
               ...
               train_mask_bool=None, test_mask_bool=None, strict_masking=False):
    ...
    for epoch in range(epochs):
        ...
        if strict_masking:
            if test_mask_bool is None:
                raise ValueError("test_mask_bool is required for strict masking")
            ppi_edge_train = filter_edges_incident_to_test(
                edge_index_train, test_mask_bool)
            semantic_edge_train = filter_edges_incident_to_test(
                L_emb_edge, test_mask_bool)
            non_test_mask = ~test_mask_bool
            mask_column = non_test_mask.unsqueeze(1).to(data.x.dtype)
            x_train = data.x * mask_column
            L_emb_train = {name: value * mask_column for name, value in L_emb.items()}
            contrastive_mask = non_test_mask
        else:
            ppi_edge_train = edge_index_train
            semantic_edge_train = L_emb_edge
            x_train = data.x
            L_emb_train = L_emb
        ...
        loss_inter, label_G, label_self, label_neighbor, label_together, label_concat, label_satment, final_output = model(
            x_train, ppi_edge_train, L_emb_train, semantic_edge_train,
...
def trainPred_k_sets(input_dim, k_sets, data, L_emb, edge_index, L_emb_edge,
    ...
    for exp_id in range(10):
        for fold_id in range(5):
            ...
            aurocs, auprcs, auc, auprc = train_test(
                ...
                exp_id=exp_id,
                fold_id=fold_id,
                train_mask_bool=train_mask,
                test_mask_bool=test_mask,
            ...
            os.makedirs(RESULT_ROOT, exist_ok=True)
            result_prefix = os.path.join(RESULT_ROOT, f'{dataset}_{EXPERIMENT_VARIANT}')
            np.savetxt(result_prefix + '_auroc.txt', list_aurocs, fmt='%.6f')
            np.savetxt(result_prefix + '_auprc.txt', list_auprcs, fmt='%.6f')
    ...
    save_best_epoch_results(
        ...
        epochs=epochs,
        output_path=os.path.join(RESULT_ROOT, f'{dataset}_{EXPERIMENT_VARIANT}_best_epoch.txt')
    )
...
# ===== 主程序 =====
cancers = ['pan-cancer']
dataset = os.environ.get('A17_DATASET', 'cpdb').lower()  # 'cpdb' or 'string'
